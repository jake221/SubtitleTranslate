...
translate_and_compose(input_file, input_srt, 'en', 'zh-CN')
input_srt


input_srt
translate_and_compose(input_file, input_srt, 'en', 'zh-CN', both=False)

input_srt
# In German language, each words separated by space, so space=True
translate_and_compose(input_file, input_srt, 'en', 'de', space=True)

input_srt
# In Japanese(Chinese, Korean), words are characters which are NOT separated by space, so space=False (default)
translate_and_compose(input_file, input_srt, 'en', 'ja')
```
